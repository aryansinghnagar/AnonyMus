...
class TestIntegration(unittest.TestCase):
    ...
    def test_offline_queue_store_and_forward(self):
        ...
        sender.emit('register_peer', {'my_queue': sender_queue_id, 'peer_queue': creator_queue_id})
        sender.get_received()

        # Push to creator's (offline) queue — expect recipient_offline_queued
        sender.emit('push_queue', {'queue_id': creator_queue_id, 'payload': 'offline_message'})
        recv = sender.get_received()
        error_events = [e for e in recv if e['name'] == 'push_queue_error']
        self.assertTrue(len(error_events) > 0, f"Expected push_queue_error, got: {recv}")
        self.assertEqual(error_events[0]['args'][0]['error'], 'recipient_offline_queued')
        sender.disconnect()

        # Creator reconnects and uses rejoin_queue to reclaim their queue_id
        creator2 = server.socketio.test_client(server.app, flask_test_client=self.client)
        ...
        payloads = [e['args'][0].get('payload') for e in recv if e['name'] == 'queue_payload']
        self.assertIn('offline_message', payloads, f"Expected offline_message in payloads, got events: {recv}")
        creator2.disconnect()
