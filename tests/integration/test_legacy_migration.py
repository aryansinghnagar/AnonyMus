...
class TestLegacyMigration(unittest.TestCase):
    ...
    def test_run_legacy_migration_startup_helper(self, mock_add_service):
        ...
        database.add_contact("friendold2345677.onion", "FriendNickname", status="accepted", my_onion_address="mainserver234567.onion")
    ...
    def test_migrate_contact_endpoint(self):
        ...
        self.assertEqual(database.get_config(f"my_pubkey_for_{old_onion}"), "mockpubkey")
        
        # Call migration endpoint
        res = self.client.post('/api/contacts/migrate', json={
            'old_address': old_onion,
            'new_address': new_onion
        })
        ...
        self.assertEqual(database.get_config(f"my_pubkey_for_{new_onion}"), "mockpubkey")
