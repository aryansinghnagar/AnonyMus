...
class TestWeek21Features(unittest.TestCase):
    ...
    def test_profiles_lifecycle(self):
        ...
        res = self.client.post(
            "/api/profiles/create", json={"display_name": "Work Profile", "hidden": 0}
        )
        self.assertEqual(res.status_code, 200)
        ...
        self.assertFalse(
            any(c["onion_address"] == "workcontactonionaddress.onion" for c in contacts)
        )
        ...
        res = self.client.post(
            "/api/profiles/unlock", json={"passphrase": "wrongpassword"}
        )
        self.assertEqual(res.status_code, 401)

        # 9. Unlock hidden profile with correct password
        res = self.client.post(
            "/api/profiles/unlock", json={"passphrase": "supersecretpass"}
        )
        self.assertEqual(res.status_code, 200)
        data = res.get_json()
    ...
    def test_message_batching(self):
        ...
        res = self.client.post(
            "/api/messages/send_batch", json={"onion_address": onion, "events": events}
        )
        self.assertEqual(res.status_code, 200)
