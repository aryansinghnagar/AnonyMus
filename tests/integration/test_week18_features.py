...
class TestWeek18Features(unittest.TestCase):
    ...
    def test_preferred_relay_api(self):
        ...
        res = self.client.post(
            "/api/settings/preferred_relay",
            json={"preferred_file_relay": "http://my-relay.onion"},
        )
        self.assertEqual(res.status_code, 200)
        ...
        self.assertEqual(res.status_code, 200)
        self.assertEqual(
            res.get_json()["preferred_file_relay"], "http://my-relay.onion"
        )
    ...
    def test_send_receipts_toggle(self):
        ...
        # Toggle receipts off
        res = self.client.post(
            "/api/contacts/update_receipts",
            json={"onion_address": onion, "send_receipts": False},
        )
        self.assertEqual(res.status_code, 200)
        self.assertTrue(res.get_json()["success"])
        ...
        res = self.client.post(
            "/api/contacts/update_receipts",
            json={"onion_address": onion, "send_receipts": True},
        )
        self.assertEqual(res.status_code, 200)
        self.assertTrue(res.get_json()["success"])
    ...
    def test_message_edits_and_deletes(self):
        ...
        res = self.client.post(
            ...
            json={"onion_address": onion, "timestamp": timestamp, "state": "read"},
        )
        self.assertEqual(res.status_code, 200)
        ...
        res = self.client.post(
            ...
            json={
                "onion_address": onion,
                "timestamp": timestamp,
                "message": "New edited message text",
            },
        )
        self.assertEqual(res.status_code, 200)
        ...
        res = self.client.get(
            f"/api/messages/edits?onion_address={onion}&timestamp={timestamp}"
        )
        self.assertEqual(res.status_code, 200)
        ...
        res = self.client.post(
            "/api/messages/delete",
            json={"onion_address": onion, "timestamp": timestamp},
        )
        self.assertEqual(res.status_code, 200)
