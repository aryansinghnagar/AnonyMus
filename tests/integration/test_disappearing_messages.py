...
class TestDisappearingMessages(unittest.TestCase):
    ...
    def test_disappearing_messages_flow(self, mock_send_onion):
        ...
        res = self.client.post('/api/messages/set_ttl', json={
    ...
    def test_message_deletion_propagation(self, mock_send_onion):
        ...
        database.save_message(contact_onion, 'me', '{"iv":"iv","ciphertext":"ct","seq":1}', timestamp)
        ...
        res = self.client.post('/api/messages/delete', json={
            'onion_address': contact_onion,
            'timestamp': timestamp
        })
        self.assertEqual(res.status_code, 200)
    ...
    def test_p2p_delete_route(self):
        ...
        timestamp = int(time.time() * 1000)
        database.save_message(contact_onion, contact_onion, '{"iv":"iv","ciphertext":"ct","seq":1}', timestamp)
        ...
        # Trigger /p2p/delete
        res = self.client.post('/p2p/delete', json={
            'sender': contact_onion,
            'timestamp': timestamp
        })
        self.assertEqual(res.status_code, 200)
