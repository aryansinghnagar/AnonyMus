...
class TestPost1Features(unittest.TestCase):
    ...
    def test_supporter_badges(self):
        ...
        res = self.client.get(
            f"/api/profile/supporter_badge/status?onion_address={onion_address}"
        )
        self.assertEqual(res.status_code, 200)
        ...
        res = self.client.post(
            "/api/profile/supporter_badge",
            json={"onion_address": onion_address, "signature": sig},
        )
        self.assertEqual(res.status_code, 200)
        self.assertTrue(res.get_json()["success"])
