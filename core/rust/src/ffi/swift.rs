...
pub fn hkdf_derive(
    ...
    crypto::hkdf::derive(&ikm, salt.as_deref(), &info, output_len as usize).map_err(|e| e.to_string())
}
...
#[uniffi::export]
pub fn ed25519_verify(public_key: Vec<u8>, message: Vec<u8>, signature: Vec<u8>) -> Result<(), String> {
...
fn ed25519_verify_internal(public_key: &[u8; 32], message: &[u8], signature: &[u8; 64]) -> Result<(), String> {
...
pub fn argon2_derive_key(password: Vec<u8>, salt: Vec<u8>) -> Result<Vec<u8>, String> {
    crypto::argon2::derive_key(&password, &salt).map(|k| k.to_vec()).map_err(|e| e.to_string())
}
