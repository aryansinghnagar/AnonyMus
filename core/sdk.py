...
class AnonyMusClient:
    ...
    def _setup_socket_handlers(self):
        ...
        def on_handshake_accepted(data):
            ...
            shared_secret = self.private_key.exchange(ec.ECDH(), peer_pubkey) if hasattr(self.private_key, 'exchange') else None
            ...
            self.save_shared_secret(onion_address, shared_secret_b64, peer_public_key_b64)
            # Sync keys
            self.sync_contacts()
    ...
    def init_contact_session(self, contact):
        ...
        self.session_ids[onion] = protocol.compute_safety_number(my_pub_b64, peer_pub_b64)
    ...
    def accept_contact(self, onion_address) -> bool:
        ...
        try:
            ...
            contacts = res_c.json()
            contact = next((c for c in contacts if c.get("onion_address") == onion_address), None)
    ...
    def save_shared_secret(self, onion_address, shared_secret_b64, peer_pub_key_b64) -> bool:
        try:
            r = self.session.get(f"{self.base_url}/")
            csrf_token = self._extract_csrf(r.text)
            headers = {"X-CSRFToken": csrf_token} if csrf_token else {}
    ...
    def send_message(self, onion_address, text) -> bool:
        ...
        try:
            ...
            self.chain_keys[onion_address]["send_chain_key"] = ratchet_step["next_chain_key"]
                ...
                for m in raw_messages:
                    ...
                    try:
                        ...
                        if sender == "me":
                            ...
                            text = self.decrypt_historical(onion_address, iv, ciphertext, seq, "me")
                        else:
                            text = self.decrypt_historical(onion_address, iv, ciphertext, seq, "peer")
    ...
    def decrypt_incoming(self, sender_onion, iv, ciphertext, seq) -> str:
        ...
        try:
            ...
            self.chain_keys[sender_onion]["recv_chain_key"] = ratchet_step["next_chain_key"]
            
            session_id = self.session_ids[sender_onion]
            return protocol.decrypt_message(ratchet_step["message_key"], iv, ciphertext, "peer", seq, session_id)
        except Exception as e:
    ...
    def decrypt_historical(self, onion_address, iv, ciphertext, seq, role) -> str:
        ...
        try:
            ...
            root_key = active_keys["send_chain_key"] if role == "me" else active_keys["recv_chain_key"]
            ...
            return protocol.decrypt_message(message_key, iv, ciphertext, role, seq, session_id)
        except Exception as e:
