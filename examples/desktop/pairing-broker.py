    ...
    def do_POST(self):
        if self.path == "/api/sync/pairing":
            ...
            try:
                ...
                self.end_headers()
                self.wfile.write(
                    b'{"success": true, "message": "Device linked and database synchronized!"}'
                )
                print(
                    f"[Pairing Broker] Successfully linked and synced DB to {self.db_file_out}"
                )
...
def start_broker():
    ...
    pairing_payload = {"ip": ip, "port": port, "k": pub_b64}
