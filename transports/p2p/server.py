        ...
        def wrapper(*args, **kwargs):
            ...
            for field, expected_type in required_fields.items():
                ...
                if expected_type == int:
                    ...
                    return jsonify({"error": f"Field {field} has invalid type (expected {expected_type.__name__})"}), 400
...
def login():
    ...
    if res.get('success'):
        ...
        import hashlib
        db_key = hashlib.pbkdf2_hmac('sha256', password.encode('utf-8'), salt, iterations)
...
def add_contact():
    ...
    profile_id = session.get('active_profile_id', 'default')
    database.add_contact(onion, nickname, status='pending_outgoing', my_onion_address=my_onion, display_name=display_name, profile_id=profile_id)
...
@validate_json_schema({'onion_address': str, 'my_public_key': str, 'shared_secret': str})
def accept_contact():
...
@validate_json_schema({'onion_address': str, 'shared_secret': str, 'peer_public_key': str})
def save_secret():
    ...
    if peer_kem_public_key or my_kem_private_key:
        database.update_contact_kem_keys(onion, peer_kem_public_key=peer_kem_public_key, my_kem_private_key=my_kem_private_key)
    return jsonify({"success": True})
...
def notifications_register():
    ...
    token = _base64.urlsafe_b64encode(_secrets.token_bytes(32)).decode('utf-8').rstrip('=')
...
def accept_invite():
    ...
    try:
        ...
        my_contact_onion = database.get_config('my_onion_address') or 'unavailable.onion'
    
    import secrets
    display_name = secrets.token_hex(4)
    # Add contact with the pairwise onion address
    profile_id = session.get('active_profile_id', 'default')
    database.add_contact(invite_onion, nickname, status='pending_outgoing', my_onion_address=my_contact_onion, display_name=display_name, profile_id=profile_id)
    ...
    def do_handshake():
        ...
        if "error" in res:
            socketio.emit('contact_status_change', {"onion_address": invite_onion, "status": "offline"})
...
@validate_json_schema({'group_id': str, 'member_onion': str, 'nickname': str, 'role': str})
...
@validate_json_schema({'group_id': str, 'sender_onion': str, 'sender_nickname': str, 'message': str, 'timestamp': int})
def api_group_save_message():
    ...
    socketio.emit('group_message_saved', {
        "group_id": group_id,
...
def api_group_add_vouch():
    ...
    # Notify group UI
    socketio.emit('group_vouch_added', {
        "group_id": group_id,
...
def send_message():
    ...
    if not is_ephemeral:
        ...
        database.save_message(onion, 'me', json.dumps(message_payload), timestamp, expires_at=expires_at)
...
def send_message_batch():
    ...
    for event in events:
        ...
        if not is_ephemeral:
            ...
            database.save_message(onion, 'me', json.dumps(event_payload), timestamp, expires_at=expires_at)
...
def api_preferred_relay():
    ...
    if request.method == 'GET':
        ...
        if val and not val.startswith('http'):
            return jsonify({"error": "Preferred file relay must be a valid HTTP/HTTPS URL"}), 400
...
def p2p_handshake():
    ...
    if host and host.endswith('.onion'):
        ...
        known_onions = {c['my_onion_address'] for c in contacts if c.get('my_onion_address')}
        ...
        if known_onions and host not in known_onions:
            app.logger.warning(f"Rejected handshake: Host {host} not in known onion set")
    ...
    database.add_contact(onion, nickname, status='pending_incoming', display_name=display_name)
...
def p2p_message_batch():
    ...
    for event in events:
        ...
        if not is_valid_base64_like(iv, max_len=100) or not is_valid_base64_like(ciphertext, max_len=100000):
            continue
        ...
        if not is_ephemeral:
            message_payload = {"iv": iv, "ciphertext": ciphertext, "seq": seq}
            database.save_message(sender, sender, json.dumps(message_payload), timestamp, expires_at=expires_at)
...
@csrf.exempt
@limiter.limit("30 per minute", key_func=get_p2p_rate_limit_key)
@validate_json_schema({'sender': str, 'iv': str, 'ciphertext': str, 'seq': int, 'timestamp': int})
def p2p_message():
...
def api_create_profile():
    ...
    if hidden and passphrase:
        salt = bcrypt.gensalt()
        passphrase_hash = bcrypt.hashpw(passphrase.encode('utf-8'), salt).decode('utf-8')
...
def api_active_profile():
    ...
    profile = database.get_profile(profile_id)
    if profile:
        return jsonify({"profile_id": profile_id, "display_name": profile['display_name'], "hidden": profile['hidden']})
    return jsonify({"profile_id": "default", "display_name": "Default Profile", "hidden": 0})
...
def api_sync_push():
    ...
    try:
        ...
        peer_pub = x25519.X25519PublicKey.from_public_bytes(base64.b64decode(desktop_key_b64))
        ...
        import requests
        res = requests.post(f"http://{desktop_ip}:{desktop_port}/api/sync/pairing", json={
            "client_public_key": base64.b64encode(client_pub.public_bytes_raw()).decode('utf-8'),
            "iv": base64.b64encode(iv).decode('utf-8'),
            "ciphertext": base64.b64encode(ciphertext).decode('utf-8')
        }, timeout=15)

        if res.status_code == 200:
            return jsonify({"success": True, "message": "Database backup successfully fanned out!"})
        else:
...
def run_legacy_migration():
    ...
    try:
        ...
        main_onion = database.get_config('my_onion_address')
        for c in contacts:
            if not c.get('my_onion_address') or (main_onion and c['my_onion_address'].strip().lower() == main_onion.strip().lower()):
                import secrets
                token = secrets.token_urlsafe(12)
                service_name = f"contact_{token}"
                try:
                    ...
                    app.logger.info(f"Generated pairwise onion {new_onion} for legacy contact {c['nickname']}")
                except Exception as e:
                    app.logger.error(f"Failed to generate pairwise onion for legacy contact {c['nickname']}: {e}")
