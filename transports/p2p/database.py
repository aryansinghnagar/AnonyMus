...
def register_local_user(username, password):
    ...
    try:
        ...
        c.execute('INSERT INTO config (key, value) VALUES (?, ?)', ('db_key_salt', db_key_salt))
        conn.commit()
        success = True
    ...
    if lockout_until_str:
        try:
            ...
            if remaining > 0:
                ...
                return {"error": f"Account is locked. Try again in {int(remaining) + 1} seconds."}
        except ValueError:
            pass
...
def add_contact(onion_address, nickname, status='pending_outgoing', my_onion_address=None, display_name=None, dr_state=None, peer_kem_public_key=None, my_kem_private_key=None, profile_id='default'):
    """
    ...
    try:
        c.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (onion_address, nickname, status, my_onion_address, display_name, dr_state, peer_kem_public_key, my_kem_private_key, profile_id))
        conn.commit()
        success = True
    except Exception as e:
        success = False
...
def get_contact(onion_address, db_key=None):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT onion_address, nickname, shared_secret, peer_public_key, status, my_onion_address, disappearing_ttl, display_name, dr_state, peer_kem_public_key, my_kem_private_key, preferred_file_relay, send_receipts FROM contacts WHERE onion_address = ?', (onion_address,))
    row = c.fetchone()
    conn.close()
    if row:
        secret = row[2]
...
def get_contacts(db_key=None, profile_id='default'):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT onion_address, nickname, shared_secret, peer_public_key, status, my_onion_address, disappearing_ttl, display_name, dr_state, peer_kem_public_key, my_kem_private_key, preferred_file_relay, send_receipts FROM contacts WHERE profile_id = ?', (profile_id,))
    rows = c.fetchall()
...
def update_contact_dr_state(onion_address, dr_state):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('UPDATE contacts SET dr_state = ? WHERE onion_address = ?', (dr_state, onion_address))
    conn.commit()
    conn.close()


def update_contact_kem_keys(onion_address, peer_kem_public_key=None, my_kem_private_key=None):
    """Stores ML-KEM-768 key material for a contact (used by PQ hybrid mode)."""
    onion_address = onion_address.strip().lower()
    conn = get_connection()
    c = conn.cursor()
    if peer_kem_public_key is not None:
        c.execute('UPDATE contacts SET peer_kem_public_key = ? WHERE onion_address = ?', (peer_kem_public_key, onion_address))
    if my_kem_private_key is not None:
        c.execute('UPDATE contacts SET my_kem_private_key = ? WHERE onion_address = ?', (my_kem_private_key, onion_address))
    conn.commit()
    conn.close()
...
def update_contact_display_name(onion_address, display_name):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('UPDATE contacts SET display_name = ? WHERE onion_address = ?', (display_name, onion_address))
    conn.commit()
    conn.close()
...
def get_notify_token(onion_address: str) -> str | None:
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT notify_queue_token FROM contacts WHERE onion_address = ?', (onion_address,))
    row = c.fetchone()
    conn.close()
...
def set_notify_token(onion_address: str, token: str) -> None:
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('UPDATE contacts SET notify_queue_token = ? WHERE onion_address = ?', (token, onion_address))
    conn.commit()
    conn.close()
...
def poll_notify_queue(tokens: list[str]) -> set[str]:
    ...
    c.execute(f'SELECT DISTINCT token FROM notify_queue WHERE token IN ({placeholders})', tokens)
    rows = c.fetchall()
    conn.close()
...
def update_contact_my_onion(onion_address, my_onion_address):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('UPDATE contacts SET my_onion_address = ? WHERE onion_address = ?', (my_onion_address, onion_address))
...
def save_message(peer_onion, sender, message, timestamp, expires_at=None):
    ...
    try:
        ...
        if expires_at is None:
            c.execute('SELECT disappearing_ttl FROM contacts WHERE onion_address = ?', (peer_onion,))
...
def delete_expired_messages() -> int:
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('DELETE FROM messages WHERE expires_at IS NOT NULL AND expires_at <= ?', (now_ms,))
    deleted = c.rowcount
    conn.commit()
    conn.close()
    return deleted
...
def set_disappearing_ttl(onion_address: str, ttl_ms: int | None) -> bool:
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('UPDATE contacts SET disappearing_ttl = ? WHERE onion_address = ?', (ttl_ms, onion_address))
    updated = c.rowcount > 0
    conn.commit()
    conn.close()
    return updated
...
def delete_message_by_timestamp(peer_onion: str, timestamp: int) -> bool:
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('DELETE FROM messages WHERE peer_onion = ? AND timestamp = ?', (peer_onion, int(timestamp)))
    deleted = c.rowcount > 0
    conn.commit()
    conn.close()
    return deleted
...
def get_expired_messages():
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT id, peer_onion, timestamp FROM messages WHERE expires_at IS NOT NULL AND expires_at <= ?', (now_ms,))
    rows = c.fetchall()
    conn.close()
...
def migrate_contact_address(old_address: str, new_address: str) -> bool:
    ...
    try:
        ...
        c.execute('UPDATE contacts SET onion_address = ? WHERE onion_address = ?', (new_address, old_address))
        
        # 2. Update messages table
        c.execute('UPDATE messages SET peer_onion = ? WHERE peer_onion = ?', (new_address, old_address))
        
        # 3. Update config keys (e.g., my_pubkey_for_{old} -> my_pubkey_for_{new})
        c.execute("SELECT value FROM config WHERE key = ?", (f"my_pubkey_for_{old_address}",))
        row = c.fetchone()
        if row:
            c.execute("INSERT OR REPLACE INTO config (key, value) VALUES (?, ?)", (f"my_pubkey_for_{new_address}", row[0]))
            c.execute("DELETE FROM config WHERE key = ?", (f"my_pubkey_for_{old_address}",))
...
def get_last_sequence_number(peer_onion):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT message FROM messages WHERE peer_onion = ? ORDER BY timestamp DESC LIMIT 1', (peer_onion,))
    row = c.fetchone()
    conn.close()
    if row:
        try:
...
def update_preferred_relay(onion, url):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('UPDATE contacts SET preferred_file_relay = ? WHERE onion_address = ?', (url, onion))
    conn.commit()
    conn.close()
...
def get_preferred_relay(onion):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT preferred_file_relay FROM contacts WHERE onion_address = ?', (onion,))
    row = c.fetchone()
    conn.close()
    return row[0] if row else None
...
def update_send_receipts(onion, enabled):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('UPDATE contacts SET send_receipts = ? WHERE onion_address = ?', (val, onion))
    conn.commit()
    conn.close()
...
def update_message_text(peer_onion, timestamp, new_text):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT message FROM messages WHERE peer_onion = ? AND timestamp = ?', (peer_onion, int(timestamp)))
    row = c.fetchone()
    if row:
        old_text = row[0]
        ...
        # Update message
        c.execute('UPDATE messages SET message = ? WHERE peer_onion = ? AND timestamp = ?', (new_text, peer_onion, int(timestamp)))
        conn.commit()
    conn.close()
...
def remove_group_member(group_id, member_onion):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute('DELETE FROM group_members WHERE group_id = ? AND member_onion = ?', (group_id, member_onion))
    conn.commit()
    conn.close()
...
def get_group_members(group_id):
    ...
    rows = c.fetchall()
    conn.close()
    return [{'member_onion': r[0], 'nickname': r[1], 'invited_by': r[2], 'role': r[3], 'joined_at': r[4]} for r in rows]

def save_group_message(group_id, sender_onion, sender_nickname, message, timestamp=None):
...
def get_group_messages(group_id):
    ...
    rows = c.fetchall()
    conn.close()
    return [{'id': r[0], 'sender_onion': r[1], 'sender_nickname': r[2], 'message': r[3], 'timestamp': r[4]} for r in rows]
...
def use_group_invite(invite_token):
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT group_id, used_at FROM group_invites WHERE invite_token = ?', (invite_token,))
    row = c.fetchone()
    if row:
        group_id, used_at = row[0], row[1]
        if used_at is None:
            c.execute('UPDATE group_invites SET used_at = ? WHERE invite_token = ?', (int(time.time() * 1000), invite_token))
            conn.commit()
            conn.close()
            return group_id
    conn.close()
...
def get_member_vouches(group_id):
    ...
    rows = c.fetchall()
    conn.close()
    return [{'vouching_member': r[0], 'vouched_member': r[1], 'timestamp': r[2]} for r in rows]
...
def get_profile(profile_id):
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT profile_id, display_name, hidden, passphrase_hash FROM profiles WHERE profile_id = ?', (profile_id,))
    row = c.fetchone()
    conn.close()
    if row:
        return {'profile_id': row[0], 'display_name': row[1], 'hidden': row[2], 'passphrase_hash': row[3]}
    return None

def verify_hidden_profile(passphrase):
    conn = get_connection()
    c = conn.cursor()
    c.execute('SELECT profile_id, display_name, passphrase_hash FROM profiles WHERE hidden = 1')
    rows = c.fetchall()
    conn.close()
