...
class NetworkDiagnosticsApp:
    ...
    def toggle_disguise(self):
        ...
        self.save_settings()
        messagebox.showinfo("Disguise Mode Configured", "Disguise settings updated. Please restart the launcher to apply the change.")

    def create_widgets(self):
        # Header banner
        banner_bg = "#003366" if self.settings.get("disguise", False) else "#333333"
        banner_text = "Network Diagnostics & Loopback Interface Utility (v2.6.4)" if self.settings.get("disguise", False) else "AnonyMus Secure Messenger Controller (v2.6.4)"
    ...
    def setup_anonymus_panel(self):
        ...
        # Mode Selection
        ttk.Label(left_pane, text="Transport Mode:", font=("Segoe UI", 9, "bold")).pack(anchor=tk.W, padx=10, pady=(10, 5))
        self.mode_var = tk.StringVar(value=self.settings.get("mode", "relay"))
        
        ttk.Radiobutton(left_pane, text="Centralized Relay (Standard)", variable=self.mode_var, value="relay").pack(anchor=tk.W, padx=20, pady=2)
        ttk.Radiobutton(left_pane, text="Decentralized P2P (Tor Onion)", variable=self.mode_var, value="p2p").pack(anchor=tk.W, padx=20, pady=2)
        ...
        self.chk_disguise = ttk.Checkbutton(left_pane, text="Disguise as Windows Utility", variable=self.disguise_var, command=self.toggle_disguise)
        self.chk_disguise.pack(anchor=tk.W, padx=10, pady=(5, 10))

        # Start / Stop / Launch buttons
        self.btn_start = ttk.Button(left_pane, text="Initialize Service", command=self.start_daemon)
        self.btn_start.pack(fill=tk.X, padx=10, pady=(10, 5))
        
        self.btn_dashboard = ttk.Button(left_pane, text="Launch Chat Client", command=self.open_dashboard, state=tk.DISABLED)
        self.btn_dashboard.pack(fill=tk.X, padx=10, pady=5)
        
        self.btn_stop = ttk.Button(left_pane, text="Shutdown Service", command=self.stop_daemon, state=tk.DISABLED)
        ...
        self.lbl_status = tk.Label(right_pane, text="Status: STOPPED", font=("Segoe UI", 10, "bold"), fg="red")
        self.lbl_status.pack(anchor=tk.W, padx=10, pady=10)

        self.lbl_onion = tk.Label(right_pane, text="Onion Address: N/A", font=("Segoe UI", 9))
        self.lbl_onion.pack(anchor=tk.W, padx=10, pady=2)

        self.lbl_uptime = tk.Label(right_pane, text="Service Uptime: 00:00:00", font=("Segoe UI", 9))
        self.lbl_uptime.pack(anchor=tk.W, padx=10, pady=2)

        specs = get_system_specs()
        ttk.Separator(right_pane, orient=tk.HORIZONTAL).pack(fill=tk.X, padx=10, pady=10)
        tk.Label(right_pane, text=f"OS: {specs['OS']}", font=("Segoe UI", 8)).pack(anchor=tk.W, padx=10, pady=1)
        tk.Label(right_pane, text=f"CPU: {specs['CPU']}", font=("Segoe UI", 8)).pack(anchor=tk.W, padx=10, pady=1)
        tk.Label(right_pane, text=f"RAM: {specs['RAM']}", font=("Segoe UI", 8)).pack(anchor=tk.W, padx=10, pady=1)
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, wrap=tk.WORD, height=8, font=("Courier New", 8))
        self.log_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.log_text.insert(tk.END, "[AnonyMus] System Diagnostics Logger initialized.\n")
    ...
    def start_daemon(self):
        ...
        if daemon_active:
            messagebox.showwarning("Service Active", "The service daemon is already running.")
            return
        ...
        print(f"[Launcher] Launching backend service in profile: {active_mode.upper()} on port {active_port}...")
