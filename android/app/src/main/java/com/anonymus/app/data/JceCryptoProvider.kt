...
class JceCryptoProvider : CryptoProvider {
    override fun generateKeyPair(): KeyPair = CryptoUtils.generateKeyPair()
    ...
    ): SessionKeys = CryptoUtils.deriveSessionKeys(myPrivateKey, theirPublicKey, myPubKeyBase64, theirPubKeyBase64)

    override fun deriveChainKeys(rootKey: ByteArray): Pair<ByteArray, ByteArray> {
        val salt = ByteArray(32)
        val chainKey = hkdfDerive(rootKey, "AnonyMus-ChainKey".toByteArray(StandardCharsets.UTF_8), salt)
        val messageKey = hkdfDerive(chainKey, "AnonyMus-MessageKey".toByteArray(StandardCharsets.UTF_8), salt)
        val nextChainKey = hkdfDerive(chainKey, "AnonyMus-NextChainKey".toByteArray(StandardCharsets.UTF_8), salt)
